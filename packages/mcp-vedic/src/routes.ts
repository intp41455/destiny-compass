...
import { calculateVedic } from "./vedic-engine.js";
 ...
 *  GET  /health      健康检查
 */
export async function vedicRoutes(app: FastifyInstance) {
  app.post("/paipan", async (request, reply) => {
    ...
    const vedic = calculateVedic(correctedDate, correctedTime, lat, lng, age);
  ...
  app.get("/health", async () => ({ status: "ok", service: "mcp-vedic" }));
}
